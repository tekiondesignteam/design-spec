...
- **Name files kebab-case, no spaces, no leading `_`** (e.g. `vehicle-search.html`, `placeholder.svg`). Spaces force `%20` in every URL; leading underscores are silently dropped by GitHub Pages/Jekyll. Keep the human-readable title in `<title>` / the NavBar, not the filename.
